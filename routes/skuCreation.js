...
router.post("/", middleware.isLoggedIn, async (req, res) => {
  try {
    ...
    skuCreation(req.body, req.session.TenantToken, req.session.UserToken).then(
      (data) => {
        // Update upc counter with new upc
        Counter.findOneAndUpdate(
          { _id: "productupc" },
          { $set: { sequence_value: data.upc } }
        ).exec();
        // Submit to SKUvault
        request(
          {
          ...
          (err, response, body) => {
            if (err) {
              ...
              console.log(
                `Submitted, response: ${response.statusCode.toString()} ${
                  body.Status
                }`
              );
